...
class ApiService {
  ...
  /// Sube una imagen (en base64) al backend, que la guarda en Cloudinary.
  /// Si se envía [itemId], la nueva imagen reemplaza a la anterior del mismo
  /// item en Cloudinary. Devuelve la URL pública de la imagen subida.
  static Future<String> uploadImage(String base64Image, {String? itemId}) async {
    try {
      final response = await http
          .post(
      ...
      throw Exception(data['error'] ?? 'Error al subir la imagen');
    } catch (e) {
      throw Exception('Error: $e');
    }
  }
